...
def register_one_rgbd_pair(
    s, t, color_files, depth_files, intrinsic, with_opencv, config, camera_files=[]
):
    ...
    # If we have camera priors from ARKit, we use them for odometry instead of trying to estimate it ourselves
    if len(camera_files) == len(color_files):
        source_transform = extract_transform(camera_files[s])
        target_transform = extract_transform(camera_files[t])
        odo_init = np.matmul(target_transform, np.linalg.inv(source_transform))
...
def make_posegraph_for_fragment(
    ...
    for s in range(sid, eid):
        for t in range(s + 1, eid):
            ...
            if (
                s % config["n_keyframes_per_n_frame"] == 0
...
def integrate_rgb_frames_for_fragment(
    ...
    volume = o3d.pipelines.integration.ScalableTSDFVolume(
        voxel_length=config["tsdf_cubic_size"] / 256.0,
        sdf_trunc=0.04,
...
def make_pointcloud_for_fragment(
    path_dataset, color_files, depth_files, fragment_id, n_fragments, intrinsic, config
):
    mesh = integrate_rgb_frames_for_fragment(
        ...
        n_fragments,
        join(
            path_dataset, config["template_fragment_posegraph_optimized"] % fragment_id
        ),
        intrinsic,
        config,
    )
...
def process_single_fragment(
    fragment_id, color_files, depth_files, n_files, n_fragments, config, camera_files=[]
):
...
def run(config):
    ...
    camera_files = get_camera_file_lists(config["path_dataset"])
    print("Got camera_files ", camera_files)
    ...
    if config["python_multi_threading"] is True:
        ...
        Parallel(n_jobs=MAX_THREAD)(
            delayed(process_single_fragment)(
                fragment_id,
                color_files,
                depth_files,
                n_files,
